...
#######################################################
####### FOR HOURLY/DAILY/WEEKLY/MONTHLY BACKUPS #######
#######################################################
...
if [ $DAY_OF_MONTH -eq 1 ] && [ $HOUR_OF_DAY -eq 4 ];
then
...
if [ $DAY_OF_WEEK = $DAY_OF_WEEK_TO_KEEP ] && [ $HOUR_OF_DAY -eq 4 ];
then
...
then
	...
	find $BACKUP_DIR -maxdepth 1 -mtime +$DAYS_TO_KEEP_HOURLY -name "*-hourly" -exec rm -rf '{}' ';'
